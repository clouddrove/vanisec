...
key      = material[32..64]`}
              ...
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/clip</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`// request
{
...
{ "stored": true }`}
              ...
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/clip/open</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`// request
{ "id": "string (base64url)" }
...
{ "ciphertext": "string (base64url)", "iv": "string (base64url)" }`}
          ...
          <section>
            <h2 className={H2}>Passwordless handoff</h2>
            <div className={CARD}>
              <p className="text-clouddrove-light mb-6">
                A different shape from the endpoints above, for moving something between two devices you are
                holding rather than sending it to another person. There is no password: the receiving device
                generates an ephemeral P-256 keypair and publishes only the public half, so it is the only thing
                that can read what arrives. The server holds two public keys and a blob.
              </p>
              <p className="text-clouddrove-light mb-6 text-sm">
                The consequence worth knowing: guessing a code lets you <em>send</em> to a waiting device. It
                never lets you read what someone else sent.
              </p>

              <h3 className={H3}>1. Receiver registers</h3>
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/beam</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`// request
{ "publicKey": "string (base64url, raw P-256 point)" }

// response
{
  "code":      "4F2K-9QX1",
  "token":     "string (hex, keep it on the receiving device)",
  "expiresIn":  300
}`}
              </pre>

              <h3 className={H3}>2. Sender looks up the key</h3>
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/beam/peek</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`// request
{ "code": "4f2k-9qx1" }
...
{ "publicKey": "string (base64url)" }`}
              ...
              <p className="text-clouddrove-light mb-6 text-sm">
                Non-destructive, and safe to hand out. Derive a shared AES-GCM key from this and your own
                ephemeral keypair.
              </p>

              <h3 className={H3}>3. Sender delivers</h3>
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/beam/send</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`{
  "code":            "4F2K-9QX1",
  "ciphertext":      "string (base64url)",
  "iv":              "string (base64url)",
  "senderPublicKey": "string (base64url)"
}`}
              </pre>
              <p className="text-clouddrove-light mb-6 text-sm">
                One payload per code. A second send returns 409 rather than replacing the first, so a racing
                sender cannot swap out what the receiver is about to read.
              </p>

              <h3 className={H3}>4. Receiver polls</h3>
              <div className="mb-3">
                <span className={VERB}>POST</span>
                <span className="ml-3 font-mono text-clouddrove-dark">/api/beam/poll</span>
              </div>
              <pre className={PRE + ' mb-6'}>
{`// request
{ "code": "4F2K-9QX1", "token": "..." }

// still waiting
{ "waiting": true }

// delivered, and the code is now gone
{ "ciphertext": "...", "iv": "...", "senderPublicKey": "..." }`}
              </pre>
              <p className="text-clouddrove-light text-sm">
                Send <code className="font-mono">cancel: true</code> to drop the beam when leaving. A wrong token
                answers 404, the same as an unknown code, so polling cannot be used to find live codes. Beams
                last five minutes.
              </p>
            </div>
          </section>
